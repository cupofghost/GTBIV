  ...
  },
  {
    ...
    run: async (page, { assert }) => {
      const r = await page.evaluate(() => {
        ...
        G.mode='foot';G.started=true;G.crouching=false;p.x=spot.x;p.z=spot.z;p.y=groundH(p.x,p.z);p.punchT=0;p.kickT=0;p.climb=null;p.bailing=false;p.stunT=0;input.jx=0;input.jy=-1;camera.position.set(p.x,p.y+3,p.z+10);camera.lookAt(p.x,p.y+1,p.z);camera.updateMatrixWorld();
        input.sprint=false; updateFoot(.04); const normal=pose(); input.sprint=true; updateFoot(.04); const sprint=pose();
        ...
        refreshButtons(); const jump=$('btnJump').getBoundingClientRect(), sprintBtn=$('btnSprint').getBoundingClientRect(), enter=$('btnEnter').getBoundingClientRect();
      ...
      assert(Math.abs(r.normal.kL-r.sprint.kL)>.2&&Math.abs(r.normal.aL-r.sprint.aL)>.2&&Math.abs(r.normal.t-r.sprint.t)>.1, 'sprint pose should visibly change knees, arms, and torso');
      assert(r.visible&&!r.overlap&&r.enterBelow, 'JUMP and SPRINT should be visible side by side without overlapping controls at 800x390');
    },
  },
] };

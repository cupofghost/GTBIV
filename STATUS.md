...
|------|--------------|-------------------------|-----------|
| 2026-07-24 | Cinema Mode — `index.html` §REPLAY/CINEMA + `tests/cases/cinema-mode.test.js` | DONE, rebased on `main`. Replay screen grown into a live free-fly director: scene dropdown (intro flythrough, 4 scripted convos, jock fight, blow up a car/cop car, shoot a ped, rat mother, last-30s scrub) + HIDE HUD toggle for clean recording; director is invulnerable while filming. | Signed: Claude Code | Opus 5 | high |
...
## Archive
- Cinema Mode (Opus 4.8, high, 2026-07-24): the REPLAY screen expanded into a director interface — live free-fly camera over the running world, staged scenes, HIDE HUD for recording, old buffer scrub kept as one scene option. `tests/cases/cinema-mode.test.js` added.

...
|------|--------------|-------------------------|-----------|
| 2026-07-25 | `index.html` (CITY GLOW section, CITY facade buckets, applyDayNight), `tests/cases/city-glow.test.js` | City Glow beautification — DONE. Lit-window night swap, instanced neon signs on street-facing facades, streetlight glow pools (grade-tilted, night-only). Placement/state systems built to carry over to final assets. New suite 3/3 + boot green. | Signed: Kimi \| K3 \| medium |
...
- 2026-07-25 (Kimi): `index.html` — CITY facade buckets now collect into top-level `facadeMats` (CITY section touch, two lines) and `applyDayNight` calls `cityGlowDayNight()` (DAY/NIGHT section touch, one line) so the new CITY GLOW section can swap lit windows / neon / pools on day-night changes. CODE MAP regenerated.
